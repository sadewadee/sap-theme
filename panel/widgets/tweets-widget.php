...
class Tweets_Widget extends WP_Widget {
	...
	function Tweets_Widget()
	{
		...
		$this->WP_Widget('tweets-widget', '* Tweets', $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
		...
		if($twitter_id) {
			if(get_transient('reedwan_twitter_' . $args['widget_id']) == false) {
				$get_tweets = wp_remote_get('http://twitter.com/statuses/user_timeline/'.$twitter_id.'.json');
				$tweets = json_decode($get_tweets['body']);
				set_transient('reedwan_twitter_' . $args['widget_id'], $tweets, 60 * 5);
			}
			if(get_transient('reedwan_twitter_' . $args['widget_id'])): $tweets = array_slice(get_transient('reedwan_twitter_' . $args['widget_id']), 0, $number); ?>
			<ul class='tweets'>
